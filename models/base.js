...
const { mysql, redis } = require('../utils/db-util')
const Op = Sequelize.Op
class BaseModel {
    constructor(table_name, schema) {
        this.model = mysql.define(table_name, schema, { freezeTableName: true, timestamps: false })
        this.redis = redis
    }
    /**
     * @returns 返回实例化的sequelize模型实例
     */
    getModel() {
        return this.model
    }
    /**
     * 查询数据并做缓存
     * @param {查询redis的key} rname 
     * @param {查询数据库的promise对象} promise 
     * @returns 数据对象
     */
    async findAndCache(rname, promise) {
        let obj = await redis.get(rname)
        if (obj) {
            obj = JSON.parse(obj)
        } else {
            obj = await promise
            await redis.set(rname, JSON.stringify(obj))
        }
        return obj
    }
